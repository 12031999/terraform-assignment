...
resource "azurerm_public_ip" "pip" {
  count               = 3
  name                = "${var.id}-pip-${count.index}"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = "Static"
  domain_name_label   = "${var.id}-dns-${count.index}"
  tags                = var.tags
}

resource "azurerm_network_interface" "nic" {
  count               = 2
  name                = "${var.id}-nic-${count.index}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_virtual_machine" "vm" {
  ...
  storage_os_disk {
    name              = "${var.id}-osdisk-${count.index}"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "n01594361-linux-${count.index+1}"
